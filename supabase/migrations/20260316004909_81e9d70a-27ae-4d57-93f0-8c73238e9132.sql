...
CREATE TABLE public.message_rewrites (
  ...
  risk_flags TEXT[],
  mode TEXT NOT NULL CHECK (mode IN ('respond', 'rewrite')),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

ALTER TABLE public.message_rewrites ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own rewrites" ON public.message_rewrites FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own rewrites" ON public.message_rewrites FOR INSERT WITH CHECK (auth.uid() = user_id);
